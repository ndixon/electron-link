...
module.exports = class FileRequireTransform {
  ...
  replaceAssignmentOrDeclarationWithLazyFunction (astPath) {
    ...
    while (parentPath != null && parentPath.scope === astPath.scope) {
      ...
      if (parentNode.type === 'AssignmentExpression') {
        ...
        if (["module", "exports"].includes(assignmentLhs.name)) {
          const msg = `The reference to the module is replaced with the lazy function, but it is assigned to "module" or "exports". In some cases the bundle might not work, which you should fix manually: \n ${this.options.filePath} \n`
    ...
    const msg = `The reference to the module is replaced with the lazy function, but it was not in an assignment expression or a variable declaration. In some cases the bundle might not work, which you should fix manually: \n ${this.options.filePath} \n`
    if (this.options.strict) {
      assert.fail(msg)
    } else if (!this.options.silent) {
      console.warn(msg)
